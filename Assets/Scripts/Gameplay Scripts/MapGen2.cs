...
public class MapGen2 : MonoBehaviour
{
    ...
    public void GenerateGrid()
    {
        ...
        for (int currentRow = 0; currentRow < rows; currentRow++)
        {
            for (int currentColumn = 0; currentColumn < columns; currentColumn++)
            {
                ...
                // Scale the room to appropriate size
                Vector3 scaler = new Vector3((roomWidth / 50), 1, (roomHeight / 50));
                tempRoomObj.transform.localScale = scaler;
    ...
    void Start()
    {
        if (GameManager.Instance.MapOfDayToggle == true)
        {
