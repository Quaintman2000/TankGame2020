...
public class AiController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        playerOneTransform = GameObject.FindGameObjectWithTag("Player").GetComponent<Transform>();
    }
    ...
    void Update()
    {
        ...
        //if player one != null and it still has lives left
        if(playerOneTransform != null && GameManager.Instance.playerOneLives != 0)
        {
            playerOneTransform = GameManager.Instance.playerOneData.transform;
        }
        //same for player 2
        if(GameManager.Instance.mulitplayer == true && playerTwoTransform != null && GameManager.Instance.playerTwoLives != 0)
        {
    ...
    private void HandleAiPersonalities()
    {
        ...
        switch(personality)
        {
            ...
            case AIPersonality.Ambusher:
                AmbusherStateMachince();
                break;
            case AIPersonality.Cowardly:
                CowardlyStateMachine();
    ...
    private void AggressiveStateMachine()
    {
    ...
    private void AmbusherStateMachince()
    {
        //set the color for the personality
    ...
    private void CowardlyStateMachine()
    {
        //set the color for the personality
        ...
        {
            //if ai can see the player
            if (vision.CanSee(playerOneTransform.gameObject) == true && currentAIState == AIState.Wait)
            {
                //start the timer
                fleeTimer = fleeTime;
                currentTarget = playerOneTransform;
            ...
            else if (fleeTimer <= 0 && !(vision.CanSee(playerOneTransform.gameObject)))
            {
                ChangeState(AIState.Wait);
            }
        }
        ...
        {
            //if ai can see the player
            if (vision.CanSee(playerTwoTransform.gameObject) == true && currentAIState == AIState.Wait)
            {
                //start the timer
                fleeTimer = fleeTime;
                currentTarget = playerTwoTransform;
            ...
            else if (fleeTimer <= 0 && !(vision.CanSee(playerTwoTransform.gameObject)))
            {
                ChangeState(AIState.Wait);
            }
        }
    ...
    private void SniperStateMachine()
    {
        //set the color for the personality
        ...
        if (playerOneTransform != null)
        {
            ...
            if (vision.CanSee(playerOneTransform.gameObject) && (currentAIState == AIState.Patrol || currentAIState == AIState.Investigate))
            {
                currentTarget = playerOneTransform;
                //enter aim state
            ...
            else if (hearing.CanHear(playerOneTransform.gameObject) && currentAIState == AIState.Patrol)
            {
                currentTarget = playerOneTransform;
            ...
            else if (currentAIState == AIState.Investigate && !vision.CanSee(playerOneTransform.gameObject) && Time.time >= investigateTimer)
            {
                currentTarget = null;
                //return to patrol
                ChangeState(AIState.Patrol);
            ...
            else if (currentAIState == AIState.Aim && vision.WithinSights(playerOneTransform.gameObject) == true && vision.CanSee(playerOneTransform.gameObject))
            {
                //enter attack state
                ChangeState(AIState.Attack);
            }
            //if the AI is in aim or attack mode AND can't see the target...
            else if ((currentAIState == AIState.Aim || currentAIState == AIState.Attack) && !vision.CanSee(playerOneTransform.gameObject))
            {
                //enter investigate state
                ChangeState(AIState.Investigate);
                //set the timer
        ...
        {
            //if the AI can see the player during patrol or investigate...
            if (vision.CanSee(playerTwoTransform.gameObject) && (currentAIState == AIState.Patrol || currentAIState == AIState.Investigate))
            {
                currentTarget = playerTwoTransform;
                //enter aim state
                ChangeState(AIState.Aim);
            ...
            //if the AI can't see the player but can HEAR the player while on patrol...
            else if (hearing.CanHear(playerTwoTransform.gameObject) && currentAIState == AIState.Patrol)
            {
                currentTarget = playerTwoTransform;
                //enter investigate state
                ChangeState(AIState.Investigate);
            ...
            //if the AI still cant see while in investigate state and the timer runs out...
            else if (currentAIState == AIState.Investigate && !vision.CanSee(playerTwoTransform.gameObject) && Time.time >= investigateTimer)
            {
                currentTarget = null;
                //return to patrol
                ChangeState(AIState.Patrol);
            ...
            //if the AI is aiming and the target is within sights...
            else if (currentAIState == AIState.Aim && vision.WithinSights(playerTwoTransform.gameObject) == true && vision.CanSee(playerTwoTransform.gameObject))
            {
                //enter attack state
                ChangeState(AIState.Attack);
            }
            //if the AI is in aim or attack mode AND can't see the target...
            else if ((currentAIState == AIState.Aim || currentAIState == AIState.Attack) && !vision.CanSee(playerTwoTransform.gameObject))
            {
                //enter investigate state
                ChangeState(AIState.Investigate);
                //set the timer
